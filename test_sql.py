...
def sql_count_juchu(users, host, dbnames, passwords, staff_department_mapping, tables, start_date, end_date):
    ...
    for table, new_table_name in tables.items():
        ...
        for staff_name, eigyou_bu in staff_department_mapping.items():
            sql_query += f"count(create_staff in ({staff_name}) or null) as {eigyou_bu}, "
        ...
        # In kết quả
# Kiểm tra xem results có giá trị không
            # In kết quả
...
tables = {"offc_trn_juchu_details":"受注明細", "offc_trn_mitsumori_details":"見積明細", "offc_trn_hachu":"発注明細","offc_trn_juchu_details_check":"チェック明細"}
...
# Thực hiện SQL cho mỗi phần tử trong tập hợp staff_name
excel_path = f"C:\\Users\\DSP189\Desktop\\test\\営業本部明細入力数_{excel_date_str}.xlsx"
# Đổi tên các bảng trong từ điển kết quả
    ...
    with pd.ExcelWriter(excel_path, engine='openpyxl', mode='a') as writer:
        ...
        for table, new_table_name in tables.items():
            ...
            for staff_name, eigyou_bu in staff_department_mapping.items():
                ...
                sheet_name = f"{new_table_name} {eigyou_bu}"
                print("d")
